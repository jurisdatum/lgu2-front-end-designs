<?php 
	function isCurrentPage($snippet)
	{
		$currentPage = pathinfo($_SERVER["PHP_SELF"])["filename"];
        if(strpos(strtolower($currentPage), $snippet) !== false)
        {
			return true;
        }
		else
...
?>
	<header>
		<section>
			<ul class="skip-links">
				<li><a href="#main-content-h">Skip to content</a></li>
					...
					<div class="hdr-links">
						<nav aria-labelledby="mainMenu" class="main-menu">
							<ul>
								...
								<li><a href="/tna/research-tools.php"<?php if(isCurrentPage("research")) { ?> aria-current="page"<?php } ?>>Research tools</a></li>
								<li><a href="/tna/help-and-guidance.php"<?php if(isCurrentPage("help")) { ?> aria-current="page"<?php } ?>>Help and guidance</a></li>
								<li><a href="/tna/whats-new.php"<?php if(isCurrentPage("new")) { ?> aria-current="page"<?php } ?>>What's new</a></li>
		...
		<section class="hdr-search">
			<h2>Search for legislation</h2>
			<details open>
				<summary><span>Search</span></summary>
				<form role="search" method="get" action="/tna/search.php">
					<fieldset>
						<legend>Legislation search</legend>
						<span class="title">
							<label for="title">Words in legislation title:</label>
							<input type="text" id="title" name="title" placeholder="e.g. equality 2010">
						</span>
						<span>
							<span class="year">
							...
							<span class="number">
								...
								<input type="text" id="number" name="number" placeholder="e.g. 15">
							</span>
						</span>
						<span class="type">
							<label for="type">Type of legislation:</label>
							<select id="type" name="type">
								<option value="primary+secondary" selected="selected">All UK Legislation (excluding originating from the EU)</option>
								<option value="all">All Legislation (including originating from the EU)</option>
								...
								<optgroup label="Draft Legislation">
									<option value="draft">All Draft Legislation</option>
